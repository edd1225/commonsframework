...
{
	// 密码验证正则表达式
	public static final String REGULAR_PASSWORD = "^([0-9A-Za-z_.@-]{6,20})?$";
	...
	public static boolean isChinese(String checkString)
	{
		...
		for (int i = 0; i < checkString.length(); i++)
		{
			if ((int) checkString.charAt(i) > 256)
				return true;
		}
		return false;
	}
	 ...
	 */
	public static boolean isAccountFormat(String checkString,
			boolean isRequired)
	{
		int min = 6;
		int max = 20;
		return isAccountFormat(checkString, isRequired, min, max);
	}
